...
public class OrderReceiverListener {
	...
	public void listenPartition0(ConsumerRecord<?, ?> record) throws IOException {
		Gson gson = new Gson();
		NormalTradeOrderRequestBO orderBO = gson.fromJson(record.value().toString(), NormalTradeOrderRequestBO.class);
		logger.info("Id0: "+orderBO.toString());
	}
	...
	public void listenPartition1(ConsumerRecord<?, ?> record) throws IOException {
		Gson gson = new Gson();
		NormalTradeOrderRequestBO orderBO = gson.fromJson(record.value().toString(), NormalTradeOrderRequestBO.class);
		logger.info("Id1: "+orderBO.toString());
	}
	...
	public void listenPartition2(ConsumerRecord<?, ?> record) throws IOException {
		Gson gson = new Gson();
		NormalTradeOrderRequestBO orderBO = gson.fromJson(record.value().toString(), NormalTradeOrderRequestBO.class);
		logger.info("Id2: "+orderBO.toString());
	}
}
